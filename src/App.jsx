...
const EstadoBadge = ({ estado }) => {
  ...
  return <span className={`px-2 py-1.5 rounded-lg text-[10px] font-black uppercase shadow-sm whitespace-nowrap ${style}`}>{estado}</span>;
};
        ...
        {!esBateaOCama ? (
          <Card className="p-6 bg-white border-2 border-slate-200 shadow-xl text-slate-900 text-left">
            ...
            <div className="space-y-10">
              {/* Bloque Preventiva - Texto Negro Intenso (maximum contrast) */}
              <div className="relative">
                ...
                {!s.estPrev.includes("LECTURA") && !s.estPrev.includes("PREV") && (
                   <p className="text-[11px] font-black text-black mt-2 uppercase">Faltan: {fmt(s.restPrev)} {unit}</p>
                )}
              ...
              {/* Bloque General - Texto Negro Intenso (maximum contrast) */}
              <div className="border-t-2 border-slate-100 pt-8 relative">
                ...
                {!s.estGen.includes("LECTURA") && !s.estGen.includes("GEN") && (
                   <p className="text-[11px] font-black text-black mt-2 uppercase">Faltan: {fmt(s.restGen)} {unit}</p>
                )}
...
export default function AppMantenciones(){
  ...
  const calcularEstado = useCallback((e)=>{
    ...
    if (!e.horaActual || e.horaActual === 0) { estPrev = "⚠️ INGRESAR LECTURA"; estGen = "⚠️ INGRESAR LECTURA"; }
    ...
    // Validación General
    if (estGen !== "⚠️ INGRESAR LECTURA") {
      if (!e.ultimaGeneralHora || e.ultimaGeneralHora === 0) { estGen = "🛠️ CONFIG. GEN"; }
      else {
    ...
    return (
      <div className="p-6 max-w-6xl mx-auto min-h-screen bg-slate-50 font-sans text-slate-900 text-left">
        <header className="mb-10 flex justify-between items-end border-b-4 border-slate-200 pb-6">
          <div><h1 className="text-5xl font-black tracking-tighter uppercase italic text-black leading-none">VIA 5</h1><p className="font-black text-blue-600 uppercase text-[11px] tracking-widest mt-2">GESTIÓN FLOTA VIA 5</p></div>
        </header>
    ...
    <div className="p-6 max-w-7xl mx-auto min-h-screen bg-slate-50 text-slate-900 text-left">
      <div className="mb-8 flex justify-between items-center bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
        ...
        <div className="text-right">
          <p className="text-[11px] font-black text-blue-600 uppercase tracking-widest leading-none">EXPEDIENTE DE ACTIVO</p>
